%\documentclass[numberedappendix,iop,12pt]{emulateapj}
%\documentclass{report}
\documentclass[11pt]{article}

\usepackage{geometry}
\usepackage{adjustbox }
\usepackage{tikz}
\usetikzlibrary{matrix}

\geometry{a4paper, %total={172mm,257mm},
left=20mm, top=20mm, right=20mm, bottom=20mm}

\usepackage{graphicx}% Include figure files
\graphicspath{{../Plots/TB2025 Gap/}}
\usepackage{dcolumn}% Align table columns on decimal point
\usepackage{bm}% bold math
\usepackage{epsf}
\usepackage{verbatim}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{appendix}
\usepackage{color}
\usepackage{soul}
\usepackage{pdfpages}
\usepackage{verbatim} % needed for comments
%\usepackage[normalem]{ulem} % needed for strikeout font
\usepackage{mathtools}
%\usepackage{changes}
\usepackage{amsfonts}
\usepackage{tikz}
\usepackage{relsize}
\usepackage{natbib}
%\usepackage{multicol}

\usepackage{todonotes} % for comments
\usepackage{pgffor}
\usepackage{grffile}
\usepackage{amssymb}
\usepackage{subcaption}
\usepackage{multirow}
\usepackage{placeins}
\usepackage[section]{placeins}


\usepackage{ dsfont }
\usepackage[
backend=biber,
% style=alphabetic,
style=numeric,
sorting=ynt
]{biblatex}

\addbibresource{mybib.bib}




% commands:

\newcommand{\ben}[1]{\todo[inline,color=orange!20!white]{\textbf{Ben:} #1}}

\newcommand{\gal}[1]{\todo[inline,color=green!20!white]{\textbf{Gal:} #1}}

\newcommand{\ket}[1]{| #1 \rangle}
\newcommand{\bra}[1]{\langle #1 |}
\newcommand{\braket}[3][]{\langle #2 | #3 \rangle}




\usepackage{natbib}
\usepackage{tikz}
\usepackage{xcolor}
\definecolor{darkgreen}{rgb}{0.0,0.5,0.0}
%\usepackage[colorlinks,citecolor=darkgreen,dvipdfm]{hyperref} % Works with LaTeX, links fail in dvi.
% Add the next line only when strictly necessary, because it causes font bugs:
\usepackage[colorlinks,citecolor=darkgreen]{hyperref} % Works with PDFLaTeX; use for arXiv and ShareLaTeX.

\usepackage[makeroom]{cancel}


\usepackage{hyperref}% http://ctan.org/pkg/hyperref
\hypersetup{%
  colorlinks = true,
  linkcolor  = black
}



\newcommand{\DrawFig}[1]{{#1}}
%\newcommand{\DrawFig}[1]{{}}


\def\mybib#1{./#1}


\newcommand{\cfixme}[1]{{\textcolor{red}{#1}}}
% Turn fixme, Fixme, cfixme  off before submission, e.g.
%\newcommand{\Fixme}[1]{{}}

\newcommand{\Ben}[1]{{\textcolor{orange}{[#1]}}}


\newcommand{\BoldA}[1]{{\textbf{\boldmath{#1}}}}
\newcommand{\energain}{g}
\newcommand{\zeroeqconst}{\zeta}
\newcommand{\myPret}{P_{\tiny\text{ret}}}
\newcommand{\mysiso}{s_{\tiny\text{iso}}}
\newcommand{\myPesc}{P_{\tiny\text{esc}}}
\newcommand{\RPWN}{R_{\mbox{\tiny PWN}}}
\newcommand{\qdinf}{q_{\infty}}

\newcommand{\aaps}{AAPS}

\usepackage[utf8]{inputenc}














\begin{document}


\title{
    \vspace{4.0cm}
    \large{\bf{F-CAL LUXE}\\ \vspace{0.3cm}
        {\large }
        \vspace{2.0cm}
    }
}

\author{Ben David Talmor   \\\\
    Instructor: Prof. Halina Abramowicz, Prof. Yan Benhammou\\\\
    Tel Aviv University, School of High Energy and Particle Physics}
\date{2025}

\maketitle

\vspace{4.0cm}
% \begin{abstract}
% \end{abstract}






\newpage
\tableofcontents
\newpage





\section{Introduction}
Particle physics has been studied for a long time and many of the known models today describe the behaviors of systems under no external fields, or weak fields which can be modeled by using perturbation theory and linear approximation.In LUXE, we study Quantum Electro Dynamics(QED) under strong external fields, where the behavior becomes non-perturbative. In this regime we investigate some special phenomena.

One main goal of the LUXE experiment is to study the \textbf{nonlinear Compton (NLC) process} where an electron or positron absorbs n optical photons, $\gamma_L$, from laser background and converts them into a single, high-energy gamma photon.

\begin{equation}
    e^{\pm} + n \gamma_L \longrightarrow e^{\pm} + \gamma
\end{equation}

Another target of the LUXE experiment is the \textbf{nonlinear Breit-Wheeler process} where high energy photons produces an electron-positron pair.

\begin{equation}
    \gamma + n \gamma_L \longrightarrow e^+ + e^-
\end{equation}

To study this realm, we will experiment with high energy electron beam and high voltage lasers, the experiment system is built in collaboration with DESY where the electron accelerator is based.







\section{Experiment System}



A key operation for the experiment will be to capture the positrons created in the Breit-Wheeler process and determine their energies. To obtain this, we will use a sensor composed of layers of silicone semi conductors and Tungsten ($W_{74}$).
The silicone is divided to channels connected to external voltage. when high energy charged particles interact with the silicone it causes an ionization and an electrical charge transmittance. by converting this charge into digital signal, we can learn the trajectory of a particle in the sensor by seeing which channels were creating the signal.
The Tungsten, being a heavier material, is used in order to reduce the particles energy, as particles interact with it, they undergo an electromagnetic shower, in which they break down into more particles with lower momentum.the width of each tungsten layer is 3.3 mm  which results with the highest probability of the particle to create only two more particles.
In our sensor we will have multiple such structures of silicone and Tungsten layers, which are referred to as \textbf{Towers}. The towers will be placed next to each other in order to cover the surface area needed to capture the positrons.
\\ \\



\section{Gap Between Towers}

Between every two towers is a distance of \ben{check dist between towers} where we have a "blind spot" in the sensor. The goal of this section is to present how this blind spots affect the accuracy of the sensor.

\section{Zero Suppressed Data (ZS)}

\subsection{Run 1101, Zero Suppressed (Off the Gap)}




\begin{figure}[htbp]
    \centering
    % First subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1101/hit count colormap plane 0.png}
        \caption{First plane}
    \end{subfigure}
    \hfill
    % Second subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1101/hit count colormap plane 7.png}
        \caption{Last plane}
    \end{subfigure}
    \caption{Run 1101. Number of hits for each channel in planes 0 (first) and 7 (last). Here we see the area where the electron beam was pointed to on the sensor, as well as the deviation from it. the difference between (a) and (b) shows the growth of particle number over the planes and also the growth of the surface area of the hits. both can be explained by the electro magnetic shower of the high energy electron in the tungsten}
    \label{hit_amount_colormap_1101}
\end{figure}






\begin{figure}[htbp]
    \centering
    \foreach \i in {1,...,8}{
            \begin{subfigure}{0.3\textwidth}
                \includegraphics[width=\linewidth]{run 1101/run 1101 event 54106/evolution of event 54106 plane \i.png}
            \end{subfigure}
        }

    \caption{Run 1101. Shower of a single event, Run 1101, TLU 54106. Here each cell in the color map represents a channel in the layer of the sensor, and the presented numbers are the amplitude measured in the channel. The red dotted line marks the gap between the two towers, meaning that the right tower ends at the 11 column and the left starts at the 12 column. We see here a single electron hitting the zero plane, then it goes through the tungsten before hitting the first plane which reads 3 hits. The number of hits increases until plane 4 in this case and then starts to reduce, do to the loss of energy of the particles.}
    \label{fig:seven_subfigs}
\end{figure}







\begin{figure}[htbp]
    \centering
    % First subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1101/avg amp colormap plane 0.png}
        \caption{First plane}
    \end{subfigure}
    \hfill
    % Second subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1101/avg amp colormap plane 7.png}
        \caption{Last plane}
    \end{subfigure}
    \caption{Run 1101. Average amplitude in each channel, planes 0 (first) and 7 (last). the average was taken by the total amplitude of each channel, divided by the total amount of hits in the channel, for the specific plane. One can notice that for the channels at the area of the electron beam, the amplitudes at plane 7 are bigger than in plane 0, while on the channels located further, there is no significant change in the reading. It is important to state that there is a tungsten layer before the first silicone layer (plane 0) and this is may cause the scattering of electrons to reach the channels further from the electron beam area. however, most of the electron that reached this far lose more energy, and in average will stamp a lower amplitude than the ones in the beam area.}
\end{figure}


\ben{Check why there are readings on the first row of the left tower}







\begin{figure}[htbp]
    \centering  \includegraphics[width=0.6\linewidth]{run 1101/amp histo total event.png}
    \caption{run 1101. Histogram of the amplitudes written in the first plane. the lines shows the peaks of the histogram}
    \label{1101-amp-histo-plane-1}
\end{figure}








\FloatBarrier
The reason for the peaks in fig (\ref{1101-amp-histo-plane-1}) can be explained by the Minimum Ionizing particle (MIP), where particles of high energy will cause a minimum amount of energy loss when passing through the silicone. The amount of lost energy in such case will be translated to a digital signal with amplitude of around 15 ADC. Since the electrons also undergo a shower in tungsten before reaching the first plane, the most probable outcomes are of either 1 particle leaving 1 MIP - around 15 amp or after multiplying in the shower we will have 2 leaving 2 MIP - 30 amp or 3 particles leaving 3 MIP - 45 amp. which explains the peaks at 17, 28 and 42.



\begin{figure}[htbp]
    \centering  \includegraphics[width=\linewidth]{run 1101/avg total amp for each plane.png}
    \caption{Run 1101. Average amp in each plane. The blue plot describe the average amplitude over the total amount of events in the run (absolute average amp), showing how much energy did each plane read over the entire run. The orange plot shows the average amplitude over the amount of events read in the plane(relative average amp), this shows the average energy of each plane solely without comparing it to the other planes. We can on the right the amount of hits in each plane, as expected, it grows bigger as the particle goes deeper in the sensor, However, in plane 4 there was probably some malfunction in the sensor as we see a major drop in comparison to the other planes. As for the energies, we see that the average amp per plane grows bigger until plane 5 and then starts to descend. The absolute average amp behave similarly, except for plane 4 which have significantly less hits and therefore lower amplitude.}
    \label{avg amp per plane run 1101}
\end{figure}




\begin{figure}[htbp]
    \centering  \includegraphics[width=0.6\linewidth]{run 1101/percent of events with different numbers of empty first planes.png}
    \caption{Run 1101. the percentage of events where the stated planes were the first ones to read data.}
    \label{empty first planes run 1101}
\end{figure}










\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1101/Average energy for initial column of event.png}
    \caption{Run 1101. The average shower energy of an event starting in a specific column of the sensor (X position)}
    \label{empty first planes run 1101}
\end{figure}




\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1101/Average energy for initial column of event y_8.png}
    \caption{Run 1101. The average shower energy of events starting row y = 8 VS the initial column of the shower of the sensor (X position) }
    \label{empty first planes run 1101}
\end{figure}







\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1101/Shower Energy Histo per initial column of event.png}
    \caption{Run 1101. Histograms of Shower energies for events starting in different columns}
    \label{Energy histo per X 1101}
\end{figure}





\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1101/Plane energy per column.png}
    \caption{Run 1101. average energy in each plane for event starting in different columns}
    \label{empty first planes run 1101}
\end{figure}



































\FloatBarrier
\newpage
\subsection{Run 1063, Zero Suppressed - On the Gap}




\begin{figure}[htbp]
    \centering
    % First subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1063/hits colormap plane 0.png}
        \caption{First plane}
    \end{subfigure}
    \hfill
    % Second subfigure
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\linewidth]{run 1063/hits colormap plane 7.png}
        \caption{Last plane}
    \end{subfigure}
    \caption{run 1063. Number of hits for each channel in planes 0 (first) and 7 (last). Here we see the area where the electron beam was pointed to on the sensor, as well as the deviation from it. the difference between (a) and (b) shows the growth of particle number over the planes and also the growth of the surface area of the hits, both can be explained by the electro magnetic shower of the high energy electron in the tungsten}
    \label{hit amount colormap}
\end{figure}





\begin{figure}[htbp]
    \centering
    \foreach \i in {1,...,8}{
            \begin{subfigure}{0.3\textwidth}
                \includegraphics[width=\linewidth]{run 1063/run 1063 event 132119/evolution of event 132119 plane \i.png}
            \end{subfigure}
        }

    \caption{Run 1101. Shower of a single event, Run 1101, TLU 54106. Here each cell in the color map represents a channel in the layer of the sensor, and the presented numbers are the amplitude measured in the channel. The red dotted line marks the gap between the two towers, meaning that the right tower ends at the 11 column and the left starts at the 12 column. We see here a single electron hitting the zero plane, then it goes through the tungsten before hitting the first plane which reads 3 hits. The number of hits increases until plane 4 in this case and then starts to reduce, do to the loss of energy of the particles.}
    \label{fig:seven_subfigs}
\end{figure}











\begin{figure}[htbp]
    \centering  \includegraphics[width=0.6\linewidth]{run 1063/amp histho plane 0.png}
    \caption{run 1063. Histogram of the amplitudes written in the first plane. the lines shows the peaks of the histogram}
    \label{amp histo plane 0}
\end{figure}






\begin{figure}[htbp]
    \centering  \includegraphics[width=\linewidth]{run 1063/energy per plane.png}
    \caption{run 1063. Average amp in each plane. The blue plot describe the average amplitude over the total amount of events in the run (absolute average amp), showing how much energy did each plane read over the entire run. The orange plot shows the average amplitude over the amount of events read in the plane(relative average amp), this shows the average energy of each plane solely without comparing it to the other planes. We can on the right the amount of hits in each plane, as expected, it grows bigger as the particle goes deeper in the sensor, However, in plane 4 there was probably some malfunction in the sensor as we see a major drop in comparison to the other planes. As for the energies, we see that the average amp per plane grows bigger until plane 5 and then starts to descend. The absolute average amp behave similarly, except for plane 4 which have significantly less hits and therefore lower amplitude.}
    \label{avg-amp-per-plane-run-1101}
\end{figure}




\begin{figure}[htbp]
    \centering  \includegraphics[width=0.4\linewidth]{run 1063/initial shower plane.png}
    \caption{run 1063. the percentage of events stating the shower on different planes.}
    \label{empty first planes run 1101}
\end{figure}









\FloatBarrier


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1063/Average energy for initial columns of events.png}
    \caption{run 1063. The average shower energy of an event starting in a specific column of the sensor (X position)}
    \label{AVG-energy-per-X-1063}
\end{figure}


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1063/Average energy for initial columns of events y_8.png}
    \caption{run 1063. The average shower energy of events starting in a row y = 8 VS the initial column of the shower in the sensor (X position)}
    \label{AVG-energy-per-X-1063}
\end{figure}




In figure (\ref{AVG-energy-per-X-1063}) we see the average energy of an event given the column of the first pad that was activated. This run occurs close to the Gap (between columns 11 and 12) and we see indeed how the energy is decreased for the events starting in these columns.


\FloatBarrier
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1063/Shower Energy Histo per initial column of event.png}
    \caption{run 1063. Histograms of Shower energies for events starting in different columns}
    \label{Energy-histo-per-X-1063}
\end{figure}

In figure (\ref{Energy-histo-per-X-1063}) we have the histograms of energies of all showers which activated only a single pad in the first plane, for different columns of initiation. we see a difference in the center of the peak, as for the columns closer to the gap the energies are lower. we remind that in run 1101 (fig. \ref{Energy histo per X 1101}) all energies are around 4220.




\FloatBarrier
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1063/Plane energy per column.png}
    \caption{run 1063. Average energy in each plane for event starting in different columns}
    \label{planes-energy-per-X}
\end{figure}

In figure (\ref{planes-energy-per-X}) we have the average energy in a plane, for events starting in different columns, we see from fig. (\ref{avg-amp-per-plane-run-1101}) that plane 4 was problematic and many false pads were acivated in it, and indeed get some random results in fig.\ref{planes-energy-per-X} as well. We also see a drop in plane 2 for some of the columns. There is also a drop in plane 5, this might occur because this is at a depth of the sensor where the shower is close to its peak, meaning that we also have a large amount of leakege to the Gap.














\newpage

\section{Reconstructed Data}

This is updatated data with better signal analysis and thresholds.

\subsection{Choosing Runs to Analayze the Gap}

Choosing the best runs to study the uncertainties of the sensor around the gap is a crucial step to achieve percise results. We will observe the runs that are expeeriencing the least amount of leakege, and also have the least defected pads.
\\
In the following table we will present some of the candidates runs:




\FloatBarrier
\begin{table}[]
    \begin{tabular}{|c|cc|cc|}
        \hline
        \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Run\\ Number\end{tabular}} & \multicolumn{2}{c|}{Location on Sensor} & \multicolumn{2}{c|}{Location in Logbook}                                    \\ \cline{2-5}
                                                                              & \multicolumn{1}{c|}{Column}             & Row                                      & \multicolumn{1}{c|}{X}    & Y    \\ \hline
        1079                                                                  & \multicolumn{1}{c|}{12,13}              & 10                                       & \multicolumn{1}{c|}{20.1} & 29.9 \\ \hline
        1080                                                                  & \multicolumn{1}{c|}{12,13}              & 8                                        & \multicolumn{1}{c|}{20.1} & 20.1 \\ \hline
        1081                                                                  & \multicolumn{1}{c|}{12,13}              & 6                                        & \multicolumn{1}{c|}{10.1} & 20.1 \\ \hline
        1088                                                                  & \multicolumn{1}{c|}{12,13}              & 4                                        & \multicolumn{1}{c|}{0.1}  & 20.1 \\ \hline
        1089                                                                  & \multicolumn{1}{c|}{12,13}              & 2,3                                      & \multicolumn{1}{c|}{-9.8} & 20.1 \\ \hline
        1093                                                                  & \multicolumn{1}{c|}{5,6}                & 6                                        & \multicolumn{1}{c|}{-20}  & 10.1 \\ \hline
        1094                                                                  & \multicolumn{1}{c|}{5,6}                & 6                                        & \multicolumn{1}{c|}{-20}  & 10.1 \\ \hline
    \end{tabular}
    \label{table:runs_location}
\end{table}

\\ \\
To choose our reference runs, we want the least leakege well-behaving shower, we want the center to be as far as possible from the boundaries and the gap. The dimensions of the left tower is 13 rows and 20 columns as can be seen in figure (\ref{hit_amount_colormap_1101}). Hence, the best runs will be 1093 and 1094.
\\
To choose our runs for gap analysis we want to be as close to the gap but as far from the boundaries as possible. Since the gap is between columns 11,12, according to table(\ref{table:runs_location}) the best runs will be the ones also located in the middle rows of the sensor. Therefore we will check runs 1080 and 1081.
\\ \\

Next Lets observe this runs:

\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_1_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_2_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_3_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_4_hits.png}   \\
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_5_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_6_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_7_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1080 reco/plane_8_hits.png}   \\
    \end{tabular}
    \caption{Run 1080 Reconstructed, Hits amount in Each Channel in Every Plane.}
    \label{fig:Hits_amoun_colormap_1080}
\end{figure}





\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_1_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_2_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_3_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_4_hits.png}   \\
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_5_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_6_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_7_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1081 reco/plane_8_hits.png}   \\
    \end{tabular}
    \caption{Run 1081 Reconstructed, Hits amount in Each Channel in Every Plane.}
    \label{fig:Hits_amoun_colormap_1081}
\end{figure}





\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_1_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_2_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_3_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_4_hits.png}   \\
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_5_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_6_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_7_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1093 reco/plane_8_hits.png}   \\
    \end{tabular}
    \caption{Run 1093 Reconstructed, Hits amount in Each Channel in Every Plane.}
    \label{fig:Hits_amoun_colormap_1093}
\end{figure}





\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_1_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_2_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_3_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_4_hits.png}   \\
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_5_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_6_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_7_hits.png} &
        \includegraphics[width=0.22\textwidth]{run 1094 reco/plane_8_hits.png}   \\
    \end{tabular}
    \caption{Run 1094 Reconstructed, Hits amount in Each Channel in Every Plane.}
    \label{fig:Hits_amoun_colormap_1094}
\end{figure}

\\
Comparing the runs on the gap 1080(\ref{fig:Hits_amoun_colormap_1080}) and 1081(\ref{fig:Hits_amoun_colormap_1081}) we see that in run 1080 the data from plane 3 is completly disordered. While some dead channels are ok as we can reconstruct the loss data by analyzinf their surrounding channels (as can be seen in further chapters), we will not be able to compensate such misaccuracy as in run 1080, plane 3. \textbf{Hence for our "Gap run" we shall choose run 1081}
\\ \\
For our off gap run we see a relatively simmilar performances, the main differences are the dead channel in run 1093, plane 1 which does not appear in 1094, and the dead channel in 1094, plane 8 which does not appear in 1093. Since the deposited energy in the deeper planes is higher, the dead channels in plane 8 are more problematic than the ones on the initial planes, together with the dead channel on plane 8 of run 1094 being very close to the center of the beam, \textbf{for our "Reference run" we will use run 1093}


























\subsection{statistics}

Some important note on analyzing large probabilstic data sets is to understand how to determine the accuracy and relevance of our results, when we measure many values, we want to asses the mean of this values its incertainties and other statistic parameters. We will present here how to determine such values.

Mean Value - the probability weighted average of a random variable
\begin{equation}
    \mu = E[x] = \sum _i x_1 \cdot p(x_i)
\end{equation}

If all events have equal probabilities:
\begin{equation}
    \mu = E[x] = \frac{1}{N} \sum _i x_i
\end{equation}


Standard Deviation - the distance from the mean where we have an effective quantity of events
\begin{equation}
    \sigma = std =  \sqrt{Var[x]} = \frac{1}{N} \sum _i (x_i - \mu)^2
\end{equation}

Standard Deviation Error Mean - the uncertainty of the mean value
\begin{equation}
    \sigma_{\mu} = \sigma_{\bar{x}} = \frac{\sigma}{\sqrt{n-1}}
\end{equation}

Skewness - Asymetry of the distribution
\begin{equation}
    \gamma_1 = Skewness = E[(\frac{X-\mu}{\sigma})^3]
\end{equation}
\\ \\
Some important distributions that our data follows:
\\ \\
Gaussian Distribution
\begin{equation}
    f(x)  = A \cdot e^{-\frac{(x - \mu)^2}{2\sigma^2}}
\end{equation}

In the Gaussian distribution, the mean and std values appear expilcitly.
\\ \\
Gamma Distribution
\begin{equation}
    f(x) =  A \cdot \frac{1}{\Gamma(\alpha)\,\theta^{\alpha}} x^{\alpha - 1} e^{-x / \theta}
\end{equation}

In Gamma distribution, the mean and std read:

\begin{equation}
    \mu = \alpha \cdot \theta \qquad , \qquad \sigma = \sqrt{\alpha} \cdot \theta
\end{equation}

we can rewrite in our variables

\begin{equation}
    \alpha = (\frac{\mu}{\sigma})^2 \qquad , \qquad \theta = \frac{\sigma^2}{\mu}
\end{equation}



and write the Gamma distribution in terms of $\mu$ and $\sigma$
\begin{equation}
    f(x) =  A \cdot \frac{1}{\Gamma((\frac{\mu}{\sigma})^2)\,\frac{\sigma^2}{\mu}^{(\frac{\mu}{\sigma})^2}} x^{(\frac{\mu}{\sigma})^2 - 1} e^{-x / \frac{\sigma^2}{\mu}}
\end{equation}
\\ \\



\subsection{Analyzing the Shower Energies and Positions in our Reference Run - Run 1093 Reconstructed, 8 layers of tungsten}

We will observe the data of the shower energies in 8 plane of silicone, for events starting at column 6 of the detector for our Reference Run (1093)



\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_gaussian fit_binsize_5.png}  &
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_gaussian fit_binsize_50.png} &
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_gaussian fit_binsize_500.png}  \\
    \end{tabular}
    \caption{Run 1093 Reconstructed, Histogram of shower energies of events starting at column 6, the bin size from left to right are 5, 50, 500. fit to Gaussian distribution}
    \label{fig:energies_from_column_6_gaussian_1093}
\end{figure}


\FloatBarrier
\begin{figure}[htbp]
    \centering
    \begin{tabular}{cccc}
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_Gamma_fit_binsize_5.png}  &
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_Gamma_fit_binsize_50.png} &
        \includegraphics[width=0.3\textwidth]{run 1093 reco/energy_in_column_6_Gamma_fit_binsize_500.png}  \\
    \end{tabular}
    \caption{Run 1093 Reconstructed, Histogram of shower energies of events starting at column 6, the bin size from left to right are 5, 50, 500. Fit to Gamma distribution.}
    \label{fig:energies_from_column_6_gamma_1093}
\end{figure}
\\
We see that the Gaussian fit is more accurate than the Gamma, this is because we observe the total shower energy, and since all electron arrive with the same energy of 5 GEV, the maximum deposited energy in the calorimeter is limited. Hence we do not get a right tail as in the Gamma distribution.
\\ \\
Now we can take the mean of all columns in the detector and get the relation between shower energy of events and their initial column.

\FloatBarrier
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1093 reco/shower energy vs columns all rows.png}
    \caption{run 1093 Reconstructed, Average Shower Energy vs the Initial column of an Event. Events from all rows}
    \label{energies_vs_columns_all_1093}
\end{figure}


\FloatBarrier
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]
    {run 1093 reco/shower energy vs columns at row 6.png}
    \caption{run 1093 Reconstructed, Average Shower Energy vs the Initial column of an Event. Events from row 6 only}
    \label{energies_vs_columns_all_1093}
\end{figure}






\end{document}
